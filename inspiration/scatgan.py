...
possible_models = 'mnist_dense|scat_64_2|lsun_dense|scat_64_2_2|scat_64_2_3'
...
parser.add_argument('--model', required=True, help=possible_models)
...
assert args.model in possible_models.split('|')
...
from load_h5dataset import H5Dataset, H5DatasetCached
...
def get_gradient_penalty(netD, real_data, fake_data, double_sided=True):
    ...
    #print 'interpolates', interpolates.size()
    #print 'double-sided', double_sided
    #print 'gradient size', gradients.size()
    #print 'gradient penalty size', gradient_penalty.size()
    #print 'gradient norm size', gradients.norm(2, dim=1).size()
...
# Create Models
######################
netD, netG = scatmodels.get_model(args.model, args.latent)
print netG
print netD
netD = netD.cuda()
netG = netG.cuda()
...
######################
if args.buffer:
    hd = H5DatasetCached(args.data, memory_size=args.buffer*1024*1024)
else:
    hd = H5Dataset(args.data)
hd_loader = DataLoader(hd, batch_size=args.batch_size, shuffle=True)
def infinite_data(loader):
    while True:
        for data in loader:
            yield data
data_iter = infinite_data(hd_loader)
